...
<template>
  <div :class="isDark ? 'theme-dark' : 'theme-light'" class="product-page min-h-screen transition-colors duration-500">
    ...
    <div class="mobile-sticky-footer md:hidden">
      <div class="mobile-delivery">
        <p class="delivery-day">Sunday</p>
        <p class="delivery-time">8 PM – 11 PM</p>
      </div>
      <button @click="addToCart" class="mobile-add-btn">
        Add to Cart
      </button>
    </div>
...
onMounted(async () => {
  // حمّل المنتج بالـ item_code = props.id
  // أو لو عندك المنتجات محملة في store:
  const found = productsStore.products.find(p => p.item_code === props.id)
...
.mobile-delivery { flex: 1; }
.delivery-day { font-size: 12px; color: var(--subtext); margin: 0; }
.delivery-time { font-size: 14px; font-weight: 800; color: var(--heading); margin: 0; }
.mobile-add-btn {
  flex: 1;
  padding: 12px;
  border-radius: 12px;
...
.mobile-add-btn:hover { filter: brightness(1.1); }
