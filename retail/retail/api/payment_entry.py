...
def get_unallocated_payments(customer=None, company=None, currency=None, mode_of_payment=None):
    ...
    if currency and currency != "null":
        filters["paid_from_account_currency"] = currency

    if mode_of_payment and mode_of_payment != "null":
    ...
    if (
        ...
        for mpesa_payment in data.selected_mpesa_payments:
            try:
                ...
                if isinstance(new_payment_entry, dict):
    ...
    if len(data.selected_invoices) > 0 and data.total_selected_invoices > 0:
        ...
        if len(all_payments_entry) > 0:
            ...
            all_payments_entry = sorted(
                [item for item in all_payments_entry if isinstance(item, dict)],
                key=lambda k: getdate(str(k.get("posting_date"))),
                reverse=True,
            )
            all_invoices_list = sorted(
                [item for item in data.selected_invoices if isinstance(item, dict)],
            ...
            if len(reconcile_doc.allocation)>0:
                reconcile_doc.reconcile()
            else:
                frappe.throw(_("No allocations could be made"))
